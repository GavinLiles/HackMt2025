...
var keep_shooting # for continuous shooting at targets
var shoot_target # coords to shoot at
...
func _on_vision_cone_area_body_entered(body: Node2D) -> void:
	$VisionCone2D3/VisionConeRenderer.color = Color(.8, .2, 0, .5)
	print(body.global_position)
	shoot_target = body.global_position
	shoot_at(shoot_target)
	keep_shooting = true
	print("gained interst")



func _on_vision_cone_area_body_exited(body: Node2D) -> void:
	$VisionCone2D2/VisionConeRenderer.color = Color(0, .5, .5, .5)
	keep_shooting = false
	$BulletTimer.stop()
	print("lost interest")
