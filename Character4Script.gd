...
func shoot():
	...
	locked_cord = get_global_mouse_position()  # Save last mouse position when shooting
	var bullet = bullet_scene.instantiate()
	get_parent().add_child(bullet)  # Add the bullet to the scene

	bullet.global_position = $GunMuzzle4.global_position  # Start from gun muzzle
	var shoot_direction = (locked_cord - $GunMuzzle4.global_position).normalized()
	bullet.rotation = shoot_direction.angle()  

	# Ensure the bullet has velocity
	if bullet.has_method("set_velocity"):
		bullet.set_velocity(shoot_direction * 500)  # Adjust speed as needed
